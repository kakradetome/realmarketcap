...
function calculateAverageChange(coins) {
  const validChanges = coins
    .map((coin) => coin.price_change_percentage_24h_in_currency ?? coin.price_change_percentage_24h)
    .filter((change) => typeof change === "number" && !Number.isNaN(change));
...
function renderMarketPulse(coins) {
  const marketPulseElement = document.querySelector("[data-market-pulse]");

  if (!marketPulseElement) {
    return;
  }
  ...
  const averageDayChange = calculateAverageChange(coins);
  const advancingCoins = coins.filter((coin) => {
    ...
    return typeof dayChange === "number" && dayChange > 0;
  }).length;

  marketPulseElement.innerHTML = `
    <article>
      <span>Top 100 Market Cap</span>
      <strong>${formatCurrency(totalMarketCap)}</strong>
    </article>
    <article>
    ...
    <article>
      ...
      <strong class="${getChangeClass(averageDayChange)}">${formatPercentage(averageDayChange)}</strong>
    </article>
    <article>
      <span>Advancing Assets</span>
      <strong>${advancingCoins}/${coins.length}</strong>
    </article>
  `;
}
