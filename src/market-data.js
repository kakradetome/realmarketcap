...
function getDominanceReasons(coin, totalMarketCap) {
  ...
  const marketShare = totalMarketCap > 0 ? (marketCap / totalMarketCap) * 100 : null;
  const liquidityRatio = marketCap > 0 ? (volume / marketCap) * 100 : null;
  const momentumLabel = dayChange >= 0 && weekChange >= 0 ? "positive short-term momentum" : "mixed momentum worth watching";

  return [
    `#${coin.market_cap_rank ?? "—"} by market capitalization`,
    `${formatRatio(marketShare)} of tracked top-100 market cap`,
    `${formatRatio(liquidityRatio)} volume-to-market-cap liquidity`,
    momentumLabel,
  ];
}
...
function renderDominanceLeaders(coins) {
  ...
  leaderGrid.innerHTML = scoredCoins
    .map((coin, index) => {
      ...
      const dominanceReasons = getDominanceReasons(coin, totalMarketCap);
      ...
      return `
        <article class="dominance-card" style="--card-delay: ${animationDelay}">
          ...
          <div class="dominance-reasons">
            <span>Why it stands out now</span>
            <ul>
              ${dominanceReasons.map((reason) => `<li>${escapeHtml(reason)}</li>`).join("")}
            </ul>
          </div>
        </article>
      `;
