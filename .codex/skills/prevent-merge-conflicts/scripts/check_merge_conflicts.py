...
def main() -> int:
    ...
    parser.add_argument(
        "--allow-missing-base",
        action="store_true",
        help="Allow marker-only checks when the base ref is unavailable in an isolated checkout.",
    )
    ...
    if base_ref is None:
        ...
        if args.allow_missing_base:
            print("Skipping merge-tree check because --allow-missing-base was provided.")
            print("Conflict-marker scan passed, but PR mergeability was not verified.")
            return 0

        print("Fetch or create a local main ref before opening a PR, then run this check again.")
        return 1
