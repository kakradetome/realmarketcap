...
Use the local `prevent-merge-conflicts` skill before every push and before every pull request:

```bash
python3 .codex/skills/prevent-merge-conflicts/scripts/check_merge_conflicts.py
```

Also run a marker scan before committing conflict-resolution changes:
